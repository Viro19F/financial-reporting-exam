...
(function () {
  ...
  function showList() {
    root.innerHTML = `
      ...
      ${MOCKS.map(m => {
        const timing = m.noTimer ? 'untimed' : `${m.durationMinutes} min`;
        return `
          <div class="mock-item">
            <div>
              <h3>${m.title}</h3>
              <div class="mock-meta">${m.questions.length} questions · ${timing} · ${m.source || ''}</div>
            </div>
  ...
  function startMock(id) {
    ...
    if (mock.noTimer) {
      if (!confirm(`Start "${mock.title}"? Untimed — work at your own pace.`)) {
        location.href = 'mocks.html';
        return;
      }
      root.innerHTML = `
        <div class="eyebrow" style="margin-bottom:14px">${mock.title} · Untimed</div>
        <div id="quiz"></div>
      `;
      new Quiz(mock.questions, { shuffleOptions: false }).mount(document.getElementById('quiz'));
      return;
    }
